...
const defaultFormData = {
  title: '', image: '', time: '', servings: '', category: 'main',
  difficulty: 'Easy' as 'Easy' | 'Medium' | 'Hard', ingredients: '', instructions: '',
};

const categories = ['breakfast', 'lunch', 'dinner', 'main', 'salad', 'dessert', 'snack'];
const difficulties: ('Easy' | 'Medium' | 'Hard')[] = ['Easy', 'Medium', 'Hard'];

export function RecipeFormDialog({ isOpen, onClose, editingRecipe }: RecipeFormDialogProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const recipeData: Recipe = {
      ...
      // بنزود الكلمات هنا واحنا بنسيف عشان تظهر صح في الكارت
      time: `${formData.time} min`,
      servings: `${servingsNumber} ${servingsNumber === 1 ? 'person' : 'people'}`,
    ...
    if (editingRecipe) {
      dispatch({ type: 'UPDATE_RECIPE', recipe: recipeData });
      toast.success('Recipe updated successfully');
    } else {
      dispatch({ type: 'ADD_RECIPE', recipe: recipeData });
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <div className="mt-2 flex items-center gap-4">
              ...
              <div className="flex-1">
                <Input 
                  id="image" type="file" accept="image/*" onChange={handleImageUpload} 
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Input 
                id="servings" type="number" min="1" 
                value={formData.servings} onChange={(e) => setFormData({ ...formData, servings: e.target.value })} 
          ...
          <div>
            ...
            <Textarea id="ingredients" value={formData.ingredients} onChange={(e) => setFormData({ ...formData, ingredients: e.target.value })} rows={4} placeholder="2 cups flour&#10;1 cup sugar" required />
          </div>

          <div>
            <Label htmlFor="instructions">Instructions (one step per line) *</Label>
            <Textarea id="instructions" value={formData.instructions} onChange={(e) => setFormData({ ...formData, instructions: e.target.value })} rows={4} placeholder="Preheat oven to 350°F&#10;Mix ingredients" required />
          </div>
