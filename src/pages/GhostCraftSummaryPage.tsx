...
export function GhostCraftSummaryPage() {
  ...
  useEffect(() => {
    if (!normalizedOrder && !hasRedirectedRef.current) {
      ...
      navigate("/ghostcraft", { replace: true });
    }
  ...
  const handleAddToCart = () => {
    ...
    const customProduct: Product = {
      ...
      name: `Ghost Craft: ${normalizedOrder.dishDescription}`,
      ...
      description: [
        normalizedOrder.allergies.length > 0
          ? `Allergies: ${normalizedOrder.allergies.join(", ")}`
          : "",
        normalizedOrder.dietaryPreferences.length > 0
          ? `Dietary: ${normalizedOrder.dietaryPreferences.join(", ")}`
          : "",
        `Spice Level: ${normalizedOrder.spicinessLevel}/5`,
        `Salt Level: ${normalizedOrder.saltinessLevel}/5`,
        `Portion: ${normalizedOrder.portionSize}`,
        normalizedOrder.specialInstructions
          ? `Notes: ${normalizedOrder.specialInstructions}`
          : "",
      ]
        .filter(Boolean)
        .join(" • "),
